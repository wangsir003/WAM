...
本项目已内置 Android Platform Tools（包含 ADB、Fastboot 等工具），并实现了智能 ADB 检测：

### ADB 优先级策略

1. **优先使用系统 ADB** - 如果系统环境变量中已配置 ADB，应用会直接使用系统的 ADB
2. **自动降级到内置 ADB** - 如果系统未配置 ADB，应用会自动使用内置的 ADB 工具

### 优势

1. **灵活性** - 尊重用户已有的 Android SDK 配置
2. **无冲突** - 不会与系统 ADB 版本产生冲突
3. **开箱即用** - 即使没有安装 Android SDK 也能正常使用
4. **跨电脑部署** - 复制项目到新电脑即可直接使用，无需额外配置
